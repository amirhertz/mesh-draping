...
class PointCloud(GeometryDS):

    @property
    def device(self) -> D:
        return self.vs.device

    def to(self, device: D) -> PointCloud:
    ...
    def vs_normals(self) -> T:
        if self.normals is None:
            v3d = open3d.utility.Vector3dVector(np.asarray(self.vs.clone().cpu().numpy()))
            o3pc = open3d.geometry.PointCloud()
            o3pc.points = v3d
            o3pc.estimate_normals(search_param=open3d.geometry.KDTreeSearchParamHybrid(radius=1, max_nn=30))
    ...
    @mesh.setter
    def mesh(self, mesh: T_Mesh):
        if type(mesh) is T:
            self.vs = mesh
        else:
            self.vs = mesh[0]

    def __init__(self, raw_mesh: Union[T, T_Mesh]):
        if type(raw_mesh) is T:
            self.vs = raw_mesh
        else:
            self.vs = raw_mesh[0]
        self.normals: Optional[T] = None
...
def laplacian_step(ds: MeshDS, weight: float = 1.) -> MeshDS:
    weight = min(1., max(weight, -1.))
    lp = ds.simple_laplace()
    ds.vs = ds.vs - weight * lp
    return ds


def get_sub_regions(ds: MeshDS, select: T, dist: float, global_opt: bool = False) -> List[TS]:
    num_regions = select.shape[0]
    regions = []
    if global_opt:
        vs_inds = torch.arange(ds.vs.shape[0], dtype=torch.int64, device=ds.device)
        faces_inds = torch.arange(ds.faces.shape[0], dtype=torch.int64, device=ds.device)
        regions.append((vs_inds, faces_inds))
    else:
        if select.dim() == 1:
            vs_base = ds.vs[select]
        else:
            vs_base = select
        all_dist = (vs_base[:, None, :] - ds.vs[None, :,  :]).norm(2, 2)
        all_dist = all_dist.le(dist)
        for i in range(num_regions):
            vs_inds = torch.where(all_dist[i])[0]
            faces_inds = ds.v2f[vs_inds].unique()
            faces_inds = faces_inds[faces_inds.ne(-1)]
            regions.append((vs_inds, faces_inds))
    return regions


def get_ds(mesh: Union[T, T_Mesh]) -> GeometryDS:
    if type(mesh) is T or mesh[1] is None:
...
if __name__ == '__main__':
    from utils import files_utils, transformations_tools
    r = transformations_tools.get_random_rotation(True)[0]
    name = 'rex_tri'
    path = f'../raw/{name}'
    mesh = files_utils.load_mesh(path)
    ds = MeshDS(mesh)
    angles_a = ds.vs_angles
    angles_b = ds.face_angles
    diff = angles_a.sum() - angles_b.sum()
    print('done')
